...
class Question {
  ...
  var moneyTree = [0, 1000, 5000, 10000, 50000, 75000, 125000, 250000, 500000, 500000, 500000, 500000];
  let correctNumber = 0;
  
  let currentStageAmt = document.querySelector("#currentMT").innerHTML;
  let bankMoney = 0;
  ...
  bankBut.addEventListener("click", function(){
      ...
      if (currentStageAmt == 0){
          ...
          //store money
          bankMoney = bankMoney + moneyTree[correctNumber];
          if (bankMoney > 500000) {
              bankMoney = 500000;
          }
          ...
          if (localStorage) {
              localStorage.setItem("currentAmtSaved", 0);
          }
      
          if (bankMoney == 500000) {
              clearInterval(stopInterval);
              alert("You've banked $500,000! You are moving on to the next round.");
              window.location.href="readyRound2.html";			
          }	
      }
  });
      ...
      if (bankMoney < 500000) {
          optionABut.onclick = function() {
              if (optionABut.value == corrAns) {
                ...
                str1 = moneyTree[correctNumber];
              if (localStorage) {
                  localStorage.setItem("currentAmtSaved", str1);
              }
